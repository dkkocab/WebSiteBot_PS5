...
def PlayStationDigital(myFirstName, myLastName, myEmail, myAddress, myPhone,
                       myCreditCardNum, myCreditExpireMonth, myCreditExpireYear, myCVV):
    messagebox.showinfo('Message',
                        'Bot has been turned on. Press "Okay" to continue and the Playstation bot window will close and run in the background...')
    root.destroy()
    driver = webdriver.Chrome(
        subprocess.run("find / -iname res 2>/dev/null", shell=True, capture_output=True, text=True).stdout.split("\n")[-2] if
        "linux" in platform else subprocess.run("dir C:\\chromedriver.exe /b/s"))
    driver.get('https://www.walmart.com/ip/Sony-PlayStation-5-Digital-Edition/493824815')
    order(driver, myFirstName, myLastName, myEmail, myAddress, myPhone,
          myCreditCardNum, myCreditExpireMonth, myCreditExpireYear, myCVV)


def PlayStationPhysical(myFirstName, myLastName, myEmail, myAddress, myPhone,
                        myCreditCardNum, myCreditExpireMonth, myCreditExpireYear, myCVV):
    messagebox.showinfo('Message',
                        'Bot has been turned on. Press "Okay" to continue and the Playstation bot window will close and run in the background...')
    root.destroy()
    driver = webdriver.Chrome(
        subprocess.run("find / -iname chromedriver 2>/dev/null", shell=True, capture_output=True, text=True).stdout.split("\n")[-2] if
        "linux" in platform else subprocess.run("dir C:\\chromedriver.exe /b/s"))
